...
| Project | Description |
|---------|-------------|
| [**ariannamethod.ai**](https://github.com/ariannamethod/ariannamethod.ai) | **AML — Arianna Method Language + [Janus](https://github.com/ariannamethod/ariannamethod.ai#janus--a-new-transformer-architecture).** A complete ML language and a new type of transformer. Not GPT, not Llama — Janus. Attention modulated by prophecy, information flow gated by suffering, training driven by autonomous seasonal cycles, identity decomposition (θ = ε + γ + αδ). The language defines, trains, and runs transformers natively — notorch, pure C, no Python, no PyTorch, no pip, no conda, no CUDA. Proof of concept: 0.54M smoke test converged (loss 5.55→1.47), 8.55M model training on CPU. 6000+ lines of C. 500 tests. OpenMP + BLAS. |
